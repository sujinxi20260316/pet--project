...
with st.sidebar:
    ...
    if st.session_state.search_history:
        for i, item in enumerate(st.session_state.search_history[-5:]):
            ...
            if st.button(f"🕒 {time_str} {question_display}", key=f"history_{i}", use_container_width=True):
...
with st.expander("🔍 症状快速自查向导（点击展开）", expanded=False):
    ...
    with col2:
        symptom = st.selectbox("主要症状", ["请选择", "呕吐", "拉稀", "不吃东西", "没精神", "咳嗽", "掉毛", "尿异常", "抽搐"])
